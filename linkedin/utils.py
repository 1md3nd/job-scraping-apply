...
def getJobUrlData():
    JobUrlData = ""
    try:
        file = open('../data/Found Jobs DATA - 20230703.txt', 'r')
        JobUrlData = file.readlines()
...
def removeDupJobUrlData():
    JobUrlData = ""
    try:
        file = open('../data/Found Jobs DATA - 20230703.txt', 'r')
        JobUrlData = set(file.readlines())
        file1 = open('data/NAcities_CISSP_20230701-nd.txt', 'w')
        file1.writelines(JobUrlData)
        file1.close()
    except FileNotFoundError:
        text = "FileNotFound:urlData.txt file is not found. Please run ./jobPreferances/createUrl.py first and make sure you have urlData.txt in /data folder generated."
        prRed(text)
...
def writeResultsJobList(text: str):
    timeStr = time.strftime("%Y%m%d")
    fileName = "Found Jobs DATA - " + timeStr + ".txt"  # TAKE THIS DATA IN DB
    ...
    try:
        ...
        with open("data/" + fileName, 'w', encoding="utf-8") as f:
            f.write("---- Found Jobs Data ---- created at: " + timeStr + "\n")
            f.write(
                "---- Area | Title | Job Apply Link " + "\n")
            for line in lines:
                f.write(line)
            f.write(text + "\n")
        ...
        with open("data/" + fileName, 'w', encoding="utf-8") as f:
            f.write("---- Found Jobs Data ---- created at: " + timeStr + "\n")
            f.write("---- Area | Title | Job Apply Link " + "\n")
